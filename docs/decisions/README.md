...
| [`0008-codeql-test-code-and-suppression.md`](0008-codeql-test-code-and-suppression.md) | How CodeQL false positives are handled: test code is not extracted at all (`cargo_cfg_overrides=-test`, the maintainers' documented opt-out and the default every other CodeQL language reaches through a test-file classifier), so `rust/cleartext-logging` stays on for production code; nothing is filtered after analysis; the three `#[napi]` macro-glue pointer alerts are dismissed with the reason recorded; upstream reporting was considered and declined. |
| [`0009-cross-target-floating-point-agreement.md`](0009-cross-target-floating-point-agreement.md) | How results are compared across targets, given that `wasm32` links Rust's own `libm` and the browser therefore computes projected coordinates an ULP away from the native host: an absolute tolerance in grid cells (measured worst case 9.3e-14, asserted at 1e-5), with bit-exactness kept as a property of one libm identified by a fingerprint rather than guessed from the target triple; pinning a software `libm` everywhere was rejected on cost. |
| [`0010-a-common-array-model-and-containers-as-drivers.md`](0010-a-common-array-model-and-containers-as-drivers.md) | How the workspace is layered now that it is a decoder library rather than an extension: a common array model in `core` (Zarr v3's, written from the spec), each format crate a driver over it and a first-class standalone deliverable that links nothing it did not ask for, `fieldglass-fetchplan` as manifests in and chunk plan out with kerchunk kept, and a keyed `ObjectSource` beside `ByteSource`; demoting the non-umbrella crates to internal was declined. |
