...
| [`0008-codeql-test-code-and-suppression.md`](0008-codeql-test-code-and-suppression.md) | How CodeQL false positives are handled when the tool offers no lever: Rust has no inline suppression and no `#[cfg(test)]` awareness, so `rust/cleartext-logging` stays enabled and its test-code results are dropped from the SARIF by rule — never by dismissing individual alerts, and never by disabling a query the remote seam of ADR-0005 will make relevant. |
