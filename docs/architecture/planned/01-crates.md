...
| `fieldglass-fetchplan` | 11 | `core` | **GRIB half shipped** (#461). Pure: reads `.idx` / `.index` / Zarr / kerchunk manifests and returns ranges. No I/O, no clock, no format crate: parameter-level matching goes through a `ParameterResolver` trait the umbrella implements with grib2's tables (#426), so this crate stays syntax only. The `core` edge is real but narrower than drawn: the one thing used is `ByteRange`, which `PlanRange::close` returns so a closed plan is what `ByteSource::prefetch` already takes. |
