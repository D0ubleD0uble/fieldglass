...
| `fieldglass-fetchplan` | 11 | `core` | **Shipped** — the GRIB half in #461, the chunk-addressing half in #660. Pure: reads `.idx` / `.index` / kerchunk manifests and Zarr metadata, and returns ranges. No I/O, no clock, no format crate: parameter-level matching goes through a `ParameterResolver` trait the umbrella implements with grib2's tables (#426), so this crate stays syntax only. The `core` edge is real but narrower than drawn: the one thing used is `ByteRange`, which `PlanRange::close` returns so a closed plan is what `ByteSource::prefetch` already takes. |
| `fieldglass-zarr` | 11 (re-scoped) | `core` | Chunk codecs (zstd via `ruzstd`, blosc, shuffle, v3 shard inner chunks), shipped in #657. Addressing lives in `fetchplan` (#660). **Where the store walker goes is undecided** — this row used to say "directory walking is the napi host's concern", and #658 puts it in this crate instead, which the issue's own scope wins on. The open question is not *which* crate walks but where the chunk-grid arithmetic lives once two crates need it: it is `fetchplan`'s `ZarrArrayMeta` today, and a walker here needs the same code. See #658 for the options and what each costs. |
