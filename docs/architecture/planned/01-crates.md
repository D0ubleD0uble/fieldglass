...
on all four format crates and on `core`; each format crate depends on `core`
and on no other format crate; each host depends on `fieldglass` only.

**What each new crate is for**

| Crate | Milestone | Depends on | Why it is its own crate |
| --- | --- | --- | --- |
| `fieldglass` | 11 | format crates, `core` | The umbrella and public Rust API (ADR-0006): `Session`, the plain serde-derivable DTOs, `Error`, and the conformance fixtures every host runs. |
| `fieldglass-wasm` | 11 | `fieldglass`, `fetchplan` | A cdylib for `wasm32-unknown-unknown`; buffer handoff and error mapping only. The host owns memory and fetching. |
