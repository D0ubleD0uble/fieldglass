...
does not describe the raster. The grid's own name (`N32`, `O32`, and `F32` for
the regular case eccodes also names) is not recoverable from `ni` and `nj`, so
it travels beside them the way `Nside` does (#500).
