...
- **GRIB1 second-order SPD-3 packing is now cross-validated against eccodes.** The general-extended second-order decoder already handled every spatial-differencing order (0–3); the `orderOfSPD = 3` (`grid_second_order_SPD3`) path now has an end-to-end regression test pinned to an eccodes 2.34 oracle (a fixture re-encoded from the existing SPD-2 file). Progress on #42; the `no_SPD` / `SPD1` orders decode via the same path but remain unpinned (eccodes won't encode a fixture for them), and the `row_by_row` / `constant_width` / `general_grib1` layouts still need real-world samples.
