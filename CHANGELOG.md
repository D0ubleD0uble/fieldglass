...
- **Coastlines no longer streak across the world map projections.** On Mollweide, the coastline overlay drew a thick horizontal line across the whole map at about 71°N. An island that the map data splits at the ±180° meridian — Wrangel Island, off north-east Siberia — has vertices sitting exactly on it, and those were being placed on the wrong edge of the map, so the line jumped from one edge to the other. The same slip quietly truncated the Siberian coast at the antimeridian and dropped one small island entirely, and it drew the same streak on any graticule finer than 30°. Vertices on the ±180° meridian now stay on the edge they belong to, on Mollweide, Robinson, and Equal Earth alike. The projection assumed a single spherical Earth radius (6 371 229 m) for every message. GRIB1 declares 6 367 470 m, GRIB2 declares its own shape per message, and WRF projects on 6 370 000 m — so a field could land kilometres from where it belongs. On the CMC polar-stereographic sample the far corner of the grid sat 3.5 km from where eccodes puts it, and the coastline overlay drifted from the data beneath it by the same amount. Fieldglass now reads the declared earth shape (GRIB1's flag, GRIB2's `shapeOfTheEarth`, WRF's own sphere) and projects on it, matching eccodes' point iterator across the whole grid. Oblate shapes use the spheroid's mean radius, within ~0.1 % of the true figure; true ellipsoidal projection is a follow-up. Closes #271.
