...
- **GRIB1 now decodes IEEE raw-float and matrix-of-values BDS packings.** Two new decoders sit behind `fieldglass-grib1::packing::decoder_for`, which now distinguishes the `additionalFlagPresent` flag combinations eccodes' `grib1/section.4.def` `packingType` concept keys off (previously both fell through to simple packing and would have mis-read their longer headers). `grid_ieee` stores values verbatim as big-endian IEEE-754 floats — 32-bit (`precision = 1`) and 64-bit (`precision = 2`), no reference/scale transform — and is cross-validated against eccodes 2.34 oracles; 128-bit (`precision = 3`) is rejected, matching eccodes' own `NOT_IMPLEMENTED`. `grid_simple_matrix` with `matrixOfValues = 0` (a simple-packed body behind the matrix sub-header — what eccodes emits for `packingType=grid_simple_matrix`) decodes through the scalar path and is eccodes-oracle pinned. The true `matrixOfValues = 1` form (an `NR×NC` matrix at every grid point, delimited by secondary bitmaps) decodes through a dedicated `Grib1Reader::decode_matrix_message` returning a new `MatrixField`, since it is not one value per grid point and not a single renderable 2-D panel; eccodes 2.34 can neither encode nor decode that variant (its secondary-bitmap accessor asserts out), so it is validated against the eccodes `data.grid_simple_matrix.def` + `DataG1SecondaryBitmap` / `data_apply_bitmap` accessor sources, a hand-built all-present fixture (exactly hand-computable), and unit tests on the masked-cell paths. JPEG/PNG are not defined for GRIB1 edition 1 (no encoder, no obtainable fixture) and stay out of scope. Closes #44.
- **GRIB1 second-order packing is now complete: the three classic WMO layouts decode.** `grid_second_order_row_by_row` (one group per row, per-row widths), `grid_second_order_constant_width` (explicit secondary bitmap + a single shared width), and `grid_second_order_general_grib1` (secondary-bitmap-delimited variable-length groups + per-group widths) now decode through a new `fieldglass-grib1::packing::second_order_classic` module, dispatched from `complex.rs` on the eccodes `packingType` label. These predate the ECMWF general-extended family and carry no spatial differencing; each is implemented against the eccodes `DataG1SecondOrder*Packing::unpack` reference source and the WMO No. 306 / ECMWF GRIB1 spec, and cross-validated end-to-end against an eccodes 2.34 `grib_get_data` oracle (hand-built fixtures, since eccodes won't encode these layouts). With this, every row of the README second-order packing table is ✅. Closes #42.
- **Every spatial-differencing order of GRIB1 second-order packing is now cross-validated against eccodes.** The general-extended second-order decoder already handled all four orders (0–3); each now has an end-to-end regression test pinned to an eccodes 2.34 oracle. `orderOfSPD = 3` (`grid_second_order_SPD3`) uses a fixture re-encoded from the SPD-2 file, plus a second fixture covering boustrophedonic row ordering combined with the order-3 inverse (the bit-flipped SPD-3 message, since eccodes' encoder can't produce one — ECC-1402). `orderOfSPD = 0` (`grid_second_order_no_SPD`) and `orderOfSPD = 1` (`grid_second_order_SPD1`) — which eccodes refuses to *encode* ("array too small") but happily decodes — use hand-built fixtures assembled to the eccodes `data.grid_second_order_*.def` wire layout and pinned to `grib_get_data` oracles. Part of the #42 second-order packing work completed in this release.
- **The render panel's projection picker gains Web Mercator, orthographic, and polar-stereographic targets.** Beyond `source` and `equirectangular`, the picker now offers three more reprojection targets, all sharing the source inverse map through a new `TargetProjection` trait + generic `warp` in `fieldglass-core::warp` (the per-target difference is just the output pixel → `(lat, lon)` map). **Web Mercator** distributes rows linearly in Mercator Y (poleward stretch) with the latitude extent clamped to the ±85.0511° valid band; like equirectangular it honours the manual lat/lon window and echoes its extent back. **Orthographic** ("globe view") fits the visible hemisphere to the raster as a disc centred on a chosen point (Snyder USGS PP-1395 §20), masking the back hemisphere. **Polar stereographic** renders the conformal true-shape view centred on a pole with the equator at the disc rim (Snyder §21). The two azimuthal targets are parameterised by a small preset selector rather than free-form numeric inputs — orthographic centre (`atlantic` (default, 0°N 0°E), `indian` (0°N 90°E), `pacific` (0°N 180°E), `americas` (0°N 270°E), `north_pole`, `south_pole`) and polar hemisphere (`north` (default), `south`) — and report no lat/lon-box extent. The napi `RenderOptions` gains `projectionPreset`. Closes #71.
...
- **GRIB1 polar stereographic source grids now reproject into equirectangular.** The render panel's `Equirectangular` target now supports `polar_stereo` source grids in addition to lat/lon, Gaussian, and Lambert. Closes the GRIB1 half of #45. New `PolarStereoParams` / `PolarStereoProjector` in `fieldglass-core::projection` implement the sphere polar-aspect formulas (Snyder USGS PP-1395 §21, true scale fixed at 60° per GRIB convention) with both north- and south-pole orientations. The napi `MessageMeta` surface gains `polarStereoLov` / `polarStereoDxMetres` / `polarStereoDyMetres` / `polarStereoSouthPole` fields populated from the GRIB1 §3 GDS. Warp setup detects when the projection pole falls inside the grid extent and expands the equirectangular bounds to the full 360° of longitude (clamping latitude to the relevant pole) so hemispheric grids render correctly. GRIB2 §3.20 polar stereographic grids reproject through the same path once their §3 template lands (see below).
- **GRIB2 §3 grid-definition templates 3.1, 3.10, 3.20, and 3.90 now parse, and polar-stereographic reprojection honours the latitude of true scale.** `fieldglass-grib2::gds` gains decoders for rotated latitude/longitude (3.1), Mercator (3.10), polar stereographic (3.20), and space-view perspective (3.90) — the next tier of operationally-relevant grids after the 3.0/3.30/3.40 trio — each with octet layouts derived from the eccodes `template.3.N.def` definitions. The rotated lat/lon and polar stereographic decoders are cross-validated against `grib_dump` snapshots of public eccodes sample fixtures; Mercator and space view (no public sample exists) are unit-tested against hand-built buffers. The napi `MessageMeta` surfaces the polar-stereo projection parameters (`polarStereoLov` / `polarStereoLad` / `polarStereoDxMetres` / `polarStereoDyMetres` / `polarStereoSouthPole`), so GRIB2 §3.20 grids reproject alongside GRIB1. The polar-stereographic projector now derives its pole scale factor `k₀ = (1 + sin |LaD|)/2` from the parsed latitude of true scale rather than a hardcoded 60°: GRIB2 carries `LaD` explicitly (validated against the Universal Polar Stereographic specification, true scale 81°06′52.3″ → k₀ = 0.994), while GRIB1 keeps its fixed ±60° convention unchanged. Closes #70.
...
- **Lambert grids whose `LoV` is carried in [0, 360) now reproject instead of rendering blank.** `lambert_forward` scaled `lon − LoV` by the cone constant *before* the trig, so when a query longitude in [-180, 180] (what every warp target feeds in) met a `LoV` stored in [0, 360) — as NCEP/Eta files do (e.g. `LoV = 265°E`) — the cone angle was off by `n·360°` and every point projected off the grid, leaving the equirectangular / Web Mercator / polar-stereographic views empty. The difference is now wrapped into [-180, 180] first. (The polar projector was unaffected: its `d_lon` only reaches `sin`/`cos`, which are 360°-periodic.)
- **The render panel now hides reprojection targets a grid can't use yet.** Grid types without a warp (e.g. rotated lat/lon, §3.1) or with a degenerate `Dx = Dy = 0` (some eccodes sample files) previously left all five projection options selectable, then painted a blank canvas when a reprojecting target was chosen. `MessageMeta` gains a `reprojectable` flag (mirroring the `warp_setup_for` dispatch); the picker offers only **Source projection** for such grids and shows a short note. The planar warp setups also now reject a zero `Dx`/`Dy` with a clear error rather than collapsing to a blank raster.
...
- **The render panel's `Bounds` (manual lat/lon window) control now hides for projections that ignore it.** The manual lat/lon box only applies to the lat/lon-box targets (equirectangular, Web Mercator); the azimuthal targets (orthographic, polar stereographic) frame themselves by their centre/hemisphere preset, and the source projection has no window — so the control was previously shown but inert there, silently doing nothing when set to Manual. It now appears only for the targets that honour it.
- **Overlay coastlines/graticule render cleanly at the edges of regional and manually-windowed projections.** The forward map normalised every longitude into `[lon_min, lon_min+360)`, so a vertex just *west* of a sub-global window's edge wrapped to the far right and the segment clipping in from that edge was wrongly split — a notch/gap along the border of any regional grid or manual lat/lon window. Each lat/lon-box target now maps longitude to the ±360 representative nearest its window centre (continuous past the edges), and `project_polylines` keeps a run when its bounding box overlaps the viewport (so a long segment whose endpoints both fall off-canvas but whose chord crosses the screen is still drawn). The antimeridian-seam split is now gated to targets that actually wrap (`wraps_antimeridian`), so the azimuthal targets no longer mis-split a wide chord between two visible points; a truncated overlay ring no longer fabricates a `(lat, 0)` vertex. In `fieldglass-core`, the forward direction is split into a `ForwardMap` trait so a forward-only map (the source overlay) need not implement an unused `pixel_to_lonlat`.
- **The graticule no longer double-draws the antimeridian or skews about the equator, and its spacing is bounded.** The meridian loop emitted both −180° and +180° (the same line, stroked twice); parallels started at `−90 + spacing` and came out asymmetric for spacings that don't divide 90. Meridians now exclude +180°, parallels are generated symmetrically about (and including) the equator, and the spacing is clamped to the picker's 5–90° range so a hand-typed value can't generate a runaway graticule.
- **Switching projection clears the previous overlay immediately.** The new raster was blitted before the reprojected overlay returned, so the prior projection's coastlines/graticule briefly sat over the new image; the panel now wipes the stale strokes as soon as the geometry changes.
- **Polar-stereographic and Lambert reprojection now honour the grid's scanning mode.** Both store Dx/Dy as unsigned magnitudes and carry the scan direction in the scanning-mode flags, but the projectors map `i = (x − origin)/dx`, `j = (y − origin)/dy` and so depend on the increment sign. A grid scanning north→south (or east→west) previously mapped into negative indices and rendered mostly-masked or mirrored; the napi builders now apply the scan sign to the increments — a no-op for the conventional south→north / west→east scan, so the canonical fixtures are unaffected.
- **GRIB1 second-order packing combined with a bit-map that masks grid points is now rejected with a clear error instead of silently misdecoding.** The classic second-order layouts size their group/row reads against the full grid; when a BMS bit-map drops points the packed stream holds only the present values, so `row_by_row` would read past it (shifted, wrong values) while `constant_width` / `general` failed with a confusing "P2 != grid points". The combination is detected up front in `complex.rs`; full bit-map support for second-order packing is tracked separately.
- **A failed overlay projection no longer leaves the panel's overlay permanently blank.** A projection error during overlay reprojection now comes back as a sequence-tagged `overlayError` the panel resolves and re-arms, instead of a generic error that advanced the overlay state without ever clearing it — previously the coastline/graticule could only be recovered by toggling a geometry option.
- **`PlanarGridProjector::lonlat_bbox` resolves longitude spans wider than 180°.** The single-reference unwrap mis-bounded a grid whose perimeter spans more than half the globe (a wide Lambert tile); the extent is now the minimum enclosing arc of the perimeter samples (largest empty gap), which also reproduces the existing tight antimeridian span.
- **A Web Mercator manual window lying entirely outside the ±85.0511° band is now rejected** rather than collapsing the Y span to zero and smearing every row to a single latitude.
