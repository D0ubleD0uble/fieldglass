...
- **Malformed GRIB2 files can no longer crash the reader.** A message whose bit-map or data section declared a length running past the end of the message used to panic; one whose grid dimensions disagreed with the grid's own declared point count could make decoding try to allocate gigabytes for a file that held no such data. All of these now surface a clean error. The crashes were found by a new fuzz target that exercises the GRIB2 decode path against arbitrary bytes, run as a time-boxed CI job. Closes #132.
