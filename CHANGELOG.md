...
- **Malformed GRIB2 files can no longer crash the reader.** A message whose bit-map or data section declared a length running past the end of the message used to panic instead of reporting a parse error. Both cases now surface a clean error. The crashes were found by a new fuzz target that exercises the GRIB2 decode path against arbitrary bytes, run as a time-boxed CI job. Closes #132.
