...
- **NetCDF-4 / HDF5 files written in the "latest format" now decode more chunked variables.** Value decode previously read only the version-1 B-tree chunk index, so a file written by a recent libhdf5 (the "latest format") often reported an unsupported-layout error. Fieldglass now also reads the version-4 single-chunk and fixed-array indexes — the common cases for a one-chunk variable and a fixed-shape multi-chunk variable — for both filtered and unfiltered chunks, the extensible-array index used for a variable with an unlimited dimension (unfiltered), and the implicit index used for a fixed-shape, early-allocated, unfiltered variable whose chunks are stored contiguously with no on-disk index, all checked against `h5py`. The v2-B-tree index, filtered extensible arrays, and a newer version-5 layout message remain a follow-up and still report a clean error. Part of #216.
