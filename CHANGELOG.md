...
- **Overlay lines on the ±180° meridian no longer jump to the wrong edge of the map.** A coastline or graticule vertex sitting exactly on the ±180° meridian was placed on the opposite edge of the map from the one it belongs to. On Mollweide this drew a thick horizontal line clear across the map at about 71°N, through Wrangel Island, and another across Antarctica; elsewhere the stray vertex was quietly discarded instead, which nicked the Siberian coast at the ±180° meridian, dropped one small island outright, and clipped the ends of every parallel on a graticule finer than 30°. The equirectangular and Web Mercator views dropped the same vertices on a global window. Such a vertex now stays on its own edge in every view. Note that a coastline which *crosses* the seam of a recentred world map can still streak on Mollweide above about 70°; that is a separate flaw in how the overlay decides where to break a line, and is not addressed here.
