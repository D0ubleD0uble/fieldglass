#!/usr/bin/env python3
"""Build a GRIB1 lat/lon fixture that stores its points column by column.

`jPointsAreConsecutive` (GDS octet 28, bit 3) says the message stores meridians
rather than parallels: adjacent stored points step in `j`, not in `i`. Nothing
in the repo's GRIB1 corpus set it, so `decode_message_raster` could hand back a
transposed field and every test still passed (#542). This builds the message
...
**Why a ramp and not a smooth field.** `reduced_gg_n32_smooth.grib1` is read by
eye and wants isolines. This one is read by an assertion: `10*j + i` gives every
one of the 40 points a distinct value that names its own position, so a test can
say `raster[j*ni + i] == 10*j + i` and a single misplaced point fails it. A
smooth field would let a transpose hide in the interpolation.
...
Usage:  python3 tools/build_grib1_j_consecutive_fixture.py
...
# The stock regular lat/lon GRIB1 sample, shrunk to the grid below. Its PDS is
# inherited and does not describe this synthetic field; only the decode path is
# under test.
SAMPLE = pathlib.Path("/usr/share/eccodes/samples/regular_ll_sfc_grib1.tmpl")
OUT = FIXTURES / "j_consecutive_latlon.grib1"
