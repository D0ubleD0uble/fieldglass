...
Real NOAA/NCEP RAP surface-temperature message, promoted from the operational
corpus fetched by `tools/fetch_samples.sh` (the `noaa-rap-pds` AWS Open Data
bucket, `rap.<date>/rap.t<cycle>z.awp130pgrbf00.grib2`, the `TMP:surface` field
extracted by byte range; this copy is the 2026-07-01T00Z cycle). GDS template
**3.30** (Lambert Conformal) on a 451×337 13-km CONUS grid (151,987 points),
DRS template **5.40** (`grid_jpeg`, `typeOfCompressionUsed = 0` — lossless) at
10 bits per value.

The synthetic fixtures cover JPEG 2000 on a regular lat/lon grid and other
packings on Lambert; this is the first real producer shipping the *combination*,
which is what RAP emits operationally. NOAA NCEP output is U.S. government work
in the public domain.
`rap_jpeg2000_lambert_expected.json` is its eccodes 2.34.1 value+§5 oracle,
generated by `tools/build_real_fixture_oracle.py`.
