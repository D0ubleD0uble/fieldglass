...
| Field compare (difference / sum / ratio / mean) | ✅ two messages | ✅ two messages | 🚧 reader API only; panel picker follows |
...
- **Field compare (difference maps).** For GRIB, the panel's Compare row picks a second message from the same file and an operation — A − B, B − A, A + B, mean(A, B), or A / B — and renders the combined field. The difference map is the classic way to read an anomaly, a bias, or the change between two time steps. Missing data in either field is missing in the result; the two messages must sit on the same grid. The combined field rides the normal pipeline, so projection, overlays, palette, and scaling all apply. The same combine is available for NetCDF variables through the reader API, with the NetCDF panel picker to follow.
