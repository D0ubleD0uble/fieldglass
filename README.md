...
On the render side: the colormap picker (8 maps, reversible, plus any GMT color
tables you import), linear and log10 color scaling, the point probe, and PNG
export.
...
| Grid description (lat/lon, Gaussian, polar stereo, Lambert) | ✅ | ✅ lat/lon (3.0), rotated lat/lon (3.1), Mercator (3.10), transverse Mercator (3.12), polar stereo (3.20), Lambert (3.30), regular and reduced Gaussian (3.40), Lambert azimuthal equal-area (3.140), space view (3.90), spherical harmonic (3.50), bi-Fourier (3.61–3.63), HEALPix (3.150) | ✅ regular lat/lon (CF coordinate vars), WRF (Lambert, polar stereo, Mercator, unrotated lat-lon), GOES geostationary, curvilinear (2-D `lat(y,x)`/`lon(y,x)`) |
| WMO lookups (parameter, centre, level type) | ✅ ON388 Table 2 + every ECMWF local table · centres from the WMO Common Code Tables | ✅ Code Table 4.2 complete (1,387 parameters across 60 discipline/category tables) + tables 0.0 / 1.2 / 1.3 / 1.4 / 3.1 / 3.2 / 4.1 / 4.3 / 4.4 / 4.5 / 4.6 / 4.10 · centres from the WMO Common Code Tables · local tables for ECMWF, DWD and NCEP | n/a |
...
| Render-panel projection picker (8 targets) | ✅ | ✅ (any decodable message) | ✅ regular lat/lon + WRF / GOES geostationary + curvilinear |
| Field compare (difference / sum / ratio / mean) | ✅ two messages | ✅ two messages | ✅ two slices of a variable |
| Map overlays (coastlines, borders, lakes, rivers, lat/lon grid) | ✅ | ✅ (any decodable message) | ✅ regular lat/lon + WRF / GOES geostationary + curvilinear |
| Contour lines | ✅ lat/lon family + projected grids | ✅ lat/lon family + projected grids | ✅ regular lat/lon + WRF projections + curvilinear |
| Resampling picker (nearest / bilinear) | ✅ | ✅ | ✅ regular lat/lon + WRF / GOES geostationary + curvilinear |
...
- **Zoom and pan.** On the equirectangular and Web Mercator targets, scroll over the map to zoom in or out around the pointer, and drag it to pan, including across the date line. The image follows the gesture straight away and is redrawn at full detail when you let go. The Bounds row gains zoom in, zoom out and Reset view buttons, and each gesture writes the new window into its latitude and longitude fields, so you can finish by typing an exact one. Clicking without dragging still reads the value under the pointer.
...
- **Imported color tables.** *Fieldglass: Import Color Table…* in the Command Palette reads a GMT color palette table (`.cpt`), the format GMT, cpt-city, the Scientific Colour Maps and cmocean publish palettes in. The table joins the Colormap menu under Imported, in every render panel including ones already open, and stays there in later sessions. Continuous and banded tables both work, with colors given as `r/g/b`, `#rrggbb`, a gray level or an X11 color name. The table's colors are spread across the display range the same way a built-in map's are; its own value range is not applied. A table using another color model (HSV, CMYK), transparency, pattern fills, or one key and color per line is refused with the line it failed on, and nothing is imported.
- **Wind and current arrows.** A GRIB file carries a vector field as two messages, so the Vectors row picks the second one: tick Arrows, choose the v component, and the arrows are drawn over the map with the colour and weight the other overlays use. Spacing is yours to set, or it fits about twenty-four arrows across the grid. The legend states what a full-length arrow means, in the field's own units. **The arrows are rotated by the projection**, because each is drawn as a geographic segment and projected like a coastline: on an orthographic globe or a polar view they curve with the map. Files that state their components along the grid's axes rather than east and north — HRRR, NAM, and Canada's polar-stereographic winds all do — are rotated through the grid's own north, and the panel says so. A cell missing either component draws no arrow.
- **Cross-sections.** The slice picker's X and Y selectors take *any* two of a variable's dimensions, so a time–latitude Hovmöller, a latitude–depth section or a time–level plot is a matter of picking the axes. A pair that is not the file's own latitude and longitude is not a map: nothing places it on the Earth, so the panel says so, puts the projection picker and the map overlays away, and labels the axes from their own coordinate arrays instead — a CF time as a date, everything else as its value with units. Files that hold nothing but cross-sections, such as a moored time series of depth against time, now draw instead of reporting an error about their depth axis.
- **Animation.** A NetCDF or Zarr render panel with a dimension to step through, such as time, shows an Animate row: first, back, play/pause, forward and last, a speed from half a step to ten steps a second, and a Loop toggle. It starts on the time axis, found from the file's CF metadata (`axis = "T"`, `standard_name = "time"`, units such as `hours since …`) or a dimension named `time`, and any other steppable dimension can be picked instead. While playing on an automatic color range, every step is colored on the range of the step you started from, so a color means the same value in every frame. Each step waits for the one before it to finish drawing, so a slow file plays slower rather than falling behind. Moving a slice slider by hand stops playback.
...
- **Parameter table coverage.** GRIB2 ships the complete WMO Code Table 4.2 — 1,387 parameters across all 60 discipline/category tables — generated from a pinned `wmo-im/GRIB2` release, with short names taken from the abbreviations NCEP publishes against the same triples. Codes above 191 are local, meaning whatever the originating centre says, so three centres' own tables ship alongside and resolve only for files from that centre: ECMWF (2,826 parameters, 13 of them gated on a declared local-table revision), DWD (213), and NCEP (479). GRIB1 carries WMO ON388 Table 2 (versions 1–3) plus all 29 ECMWF local tables (centre 98), from the IFS and ERA5 surface tables to the aerosol, chemistry and ensemble-statistics ones, and files another centre produced with ECMWF as sub-centre read those tables too. Other centres' GRIB1 local tables do not resolve (eccodes ships a few, in layouts its own reader misreads), and a field from one is shown as `Parameter centre/table/id` naming the codes no table here defines, rather than as a name. Originating centres for both editions come from the WMO Common Code Tables — 309 for GRIB2, 239 for GRIB1 — with sub-centres resolved from the centre and sub-centre codes together.
...
| `crates/fieldglass-zarr` | Zarr chunk codecs — v2's filter-and-compressor pair and v3's ordered codec chain, including blosc and its five inner compressors, the byte and bit transposes, gzip / zlib / zstd / LZ4, and v3 sharding. Performs no I/O: a chunk arrives as bytes the caller already has. |
| `crates/fieldglass-fetchplan` | Cloud-native manifests in, byte ranges out — wgrib2 `.idx`, ECMWF `.index`, kerchunk reference documents, and the Zarr chunk-grid arithmetic that spells a chunk's key. No I/O, no clock, and no format crate. |
| `crates/fieldglass` | The host-neutral API over all three readers: open bytes, decode a field, place it on the Earth, colour it. Every host is a binding of this one ([ADR-0006](docs/decisions/0006-hosts-are-bindings-over-a-plain-data-api.md)). |
...
Three crates are **not** on crates.io yet — `fieldglass`, `fieldglass-zarr` and `fieldglass-fetchplan` — so the newest work is currently reachable only by building from this repository. `fieldglass-napi` never will be published: it is a build artefact of the extension rather than a library to depend on. The browser build is a different story: it publishes to **npm** as [`@fieldglass/wasm`](https://www.npmjs.com/package/@fieldglass/wasm) on each stable tag (#466), so a web consumer needs no Rust toolchain at all.

A container is addressed in one of two ways and says which. GRIB is a stream of self-describing messages, so a message index is the whole address. NetCDF holds named variables over shared dimensions, so a field is a variable, which two of its axes are the picture, and where the caller is standing on the rest. `Session::addressing` answers which shape it is holding. Everything *after* a decode — render, probe, contours, combine, warp, palette — takes a decoded field and is the same for both.

Every crate except `fieldglass-napi` compiles for `wasm32-unknown-unknown`, and CI checks that on every pull request as well as running the decode suites on `wasm32-wasip1`, so the browser build cannot regress unnoticed. (`fieldglass-napi` does not build for wasm32 at all: it is Node's binding.) `crates/fieldglass-wasm` is that build: `./build.sh web` produces the ES module and `./pack.sh` the npm package. It binds `fieldglass`, so it opens the same three formats the umbrella does; NetCDF joined it in #662 and is what most of the bundle's growth since 0.5.0 is. Rendering there is values first: the colour table is decided once in Rust and handed to a GPU shader, with the CPU painter kept as the fallback and as the reference the shader is checked against. Measured bundle size and decode timings are in [`crates/fieldglass-wasm/README.md`](crates/fieldglass-wasm/README.md).
...
WMO ON388 / FM 92 lookup tables live in `crates/fieldglass-grib1/src/tables.rs` (GRIB1) and `crates/fieldglass-grib2/src/tables.rs` (GRIB2) — extend the tables there rather than hardcoding strings at the napi or TypeScript layer. Several tables are generated instead, and should be regenerated rather than edited by hand: ECMWF GRIB1 local parameters (every table eccodes ships) from eccodes into `tables_local.rs` by `tools/gen_grib1_local_tables.py`, which also writes the eccodes-decode oracle its test reads; the WMO GRIB2 master tables (Code Tables 4.2, 4.4, 4.5) into `tables_wmo.rs` by `tools/gen_wmo_grib2_tables.py` from a pinned `wmo-im/GRIB2` release; the originating-centre and sub-centre tables into `tables_cct.rs` (both format crates) and `cct_tables.rs` (core) by `tools/gen_wmo_cct_tables.py` from a pinned `wmo-im/CCT` release; the ECMWF and DWD GRIB2 local parameter tables into `tables_ecmf.rs` and `tables_edzw.rs` by `tools/gen_localconcepts_tables.py` from eccodes' `localConcepts`; the NCEP GRIB2 local parameter table into `tables_ncep.rs` by `tools/gen_ncep_tables.py` from a pinned wgrib2 release; and the short names for the WMO GRIB2 master parameters into `tables_wmo_short.rs` by `tools/gen_wmo_short_names.py`, from the centre-0 rows of that same wgrib2 release, because WMO publishes no abbreviations of its own. The napi-rs bindings automatically convert `snake_case` Rust field names to `camelCase` TypeScript fields.
...
- The color names an imported color table may use are the X11 color database (`rgb.txt` from X.Org `app/rgb`), © 1985, 1989, 1998 The Open Group, used under the X11 permission notice in that repository's `COPYING`.
