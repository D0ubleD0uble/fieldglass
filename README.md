...
- **Parameter table coverage.** GRIB2 ships the complete WMO Code Table 4.2 — 1,387 parameters across all 60 discipline/category tables — generated from a pinned `wmo-im/GRIB2` release, with short names taken from the abbreviations NCEP publishes against the same triples. Codes above 191 are local, meaning whatever the originating centre says, so three centres' own tables ship alongside and resolve only for files from that centre: ECMWF (2,826 parameters, 13 of them gated on a declared local-table revision), DWD (213), and NCEP (479). GRIB1 carries WMO ON388 Table 2 (versions 1–3) plus ECMWF local tables 128 and 129 (centre 98), which cover the bulk of IFS / ERA5 fields; other centres' GRIB1 local tables do not resolve, and a field from one is shown as `Parameter centre/table/id` naming the codes no table here defines, rather than as a name. Originating centres for both editions come from the WMO Common Code Tables — 309 for GRIB2, 239 for GRIB1 — with sub-centres resolved from the centre and sub-centre codes together.
...
WMO ON388 / FM 92 lookup tables live in `crates/fieldglass-grib1/src/tables.rs` (GRIB1) and `crates/fieldglass-grib2/src/tables.rs` (GRIB2) — extend the tables there rather than hardcoding strings at the napi or TypeScript layer. Several tables are generated instead, and should be regenerated rather than edited by hand: ECMWF GRIB1 local parameters (128/129) from eccodes into `tables_ecmwf.rs` by `tools/gen_ecmwf_tables.py`; the WMO GRIB2 master tables (Code Tables 4.2, 4.4, 4.5) into `tables_wmo.rs` by `tools/gen_wmo_grib2_tables.py` from a pinned `wmo-im/GRIB2` release; the originating-centre and sub-centre tables into `tables_cct.rs` (both format crates) and `cct_tables.rs` (core) by `tools/gen_wmo_cct_tables.py` from a pinned `wmo-im/CCT` release; the ECMWF and DWD GRIB2 local parameter tables into `tables_ecmf.rs` and `tables_edzw.rs` by `tools/gen_localconcepts_tables.py` from eccodes' `localConcepts`; the NCEP GRIB2 local parameter table into `tables_ncep.rs` by `tools/gen_ncep_tables.py` from a pinned wgrib2 release; and the short names for the WMO GRIB2 master parameters into `tables_wmo_short.rs` by `tools/gen_wmo_short_names.py`, from the centre-0 rows of that same wgrib2 release, because WMO publishes no abbreviations of its own. The napi-rs bindings automatically convert `snake_case` Rust field names to `camelCase` TypeScript fields.
