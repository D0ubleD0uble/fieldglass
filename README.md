...
WMO ON388 / FM 92 lookup tables live in `crates/fieldglass-grib1/src/tables.rs` (GRIB1) and `crates/fieldglass-grib2/src/tables.rs` (GRIB2) — extend the tables there rather than hardcoding strings at the napi or TypeScript layer. Several tables are generated instead, and should be regenerated rather than edited by hand: ECMWF GRIB1 local parameters (128/129) from eccodes into `tables_ecmwf.rs` by `tools/gen_ecmwf_tables.py`; the WMO GRIB2 master tables (Code Tables 4.2, 4.4, 4.5) into `tables_wmo.rs` by `tools/gen_wmo_grib2_tables.py` from a pinned `wmo-im/GRIB2` release; and the originating-centre and sub-centre tables into `tables_cct.rs` (both format crates) and `cct_tables.rs` (core) by `tools/gen_wmo_cct_tables.py` from a pinned `wmo-im/CCT` release. The napi-rs bindings automatically convert `snake_case` Rust field names to `camelCase` TypeScript fields.
