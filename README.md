...
| 5.1 — matrix of values | `grid_simple_matrix` | 🚧 Flat form | Experimental template for an `NR×NC` matrix at each grid point (e.g. 2-D wave spectra). With `matrixBitmapsPresent = 0` the §7 holds one simple-packed value per grid point (`NR`/`NC` are descriptive metadata), decoded and rendered exactly like 5.0 — cross-validated against eccodes 2.34, value for value. The true per-point matrix (`matrixBitmapsPresent = 1`, secondary bitmaps) is the variant eccodes can neither encode nor decode; it is rejected rather than mis-decoded, and is not decoded yet. |
...
| 5.50 — spectral (spherical harmonic) | `spectral_simple` | ✅ Coefficients + synthesis | The message carries spherical-harmonic coefficients (§3.50), not values on a grid: the real `(0,0)` coefficient comes from §5 and the rest are simple-packed. They decode through `Grib2Reader::decode_spectral_message` (cross-validated against eccodes on a T63 field, coefficient for coefficient). `Grib2Reader::synthesize_spectral_message` then runs the inverse spherical-harmonic transform to turn them back into a lat/lon grid — the transform no other tool in the ecosystem performs — validated against ECMWF's definitive spectral definition. Like the GRIB1 spectral form, it has its own entry point and bypasses the scalar path. |
| 5.51 — spectral complex | `spectral_complex` | ✅ Coefficients + synthesis | The ECMWF IFS form: coefficients up to a sub-truncation are stored as raw IEEE floats and the rest are simple-packed after Laplacian rescaling `(n·(n+1))^P`. Decodes through the same `decode_spectral_message` entry point (cross-validated against eccodes coefficient for coefficient on a T63 field), and synthesizes to a grid through the same inverse transform as 5.50. |
